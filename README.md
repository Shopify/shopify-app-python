...
- Python >= 3.7
- cryptography to verify requests
...
- `verify_flow_action_req`: Requests from Flow action extensions (coming soon)
...
- `app_home_redirect`: Handle redirects within embedded app home request (coming soon)
...
def shopify_result_to_response(result):
    log = result["log"]
    logger.info("%s - %s", log["code"], log["detail"])

    resp = result["response"]
    return HttpResponse(resp["body"], status=resp["status"], headers=resp['headers'])
```
...
Properties for Exchangeable ID Token Requests (`verify_app_home_req`, `verify_admin_ui_ext_req`, `verify_pos_ui_ext_req`):

| Property                | Description                                                                              | Nullable |
| ----------------------- | ---------------------------------------------------------------------------------------- | -------- |
| `user_id`               | The merchant user ID. `None` if `ok` is `False`.                                         | Yes      |
| `id_token`              | Object containing `exchangeable` (Boolean), `token` (string), and `claims` (Dictionary). | Yes      |
| `new_id_token_response` | Pre-built response for invalid token retry flow.                                         | Yes      |

Properties for App Proxy Requests (`verify_app_proxy_req`):
...
| `access_mode`           | string       | Access mode: "online" or "offline"                  |
...
| `refresh_token_expires` | string       | ISO 8601 datetime when refresh token expires        |
| `user_id`               | string       | A unique identifier for the user                    |
| `user`                  | dict or None | User details (online mode only, `None` for offline) |

When `access_mode` is "online", the `user` dict contains:
...
Replace `{{ client_id }}` with your the `SHOPIFY_API_KEY` provided by the Shopify CLI.
...
`admin_graphql_request` provides the following properties on the result:
...
- `http_logs`: List of HTTP request/response logs for debugging and monitoring.
- `data`: The GraphQL response data, or `None` if the request failed.
- `extensions`: The GraphQL extensions (e.g., cost information), or `None` if not present.
...
The `access_token` object contains:
