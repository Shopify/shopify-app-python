"""
Shopify Admin UI Extension Verification

This module provides functions to verify Shopify Admin UI Extension requests.
"""
...
def verify_admin_ui_ext_req(
    ...
    """
    Verifies requests coming from Shopify Admin UI Extensions.
        ...
        try:
            payload = jwt.decode(
                ...
                options={
                    ...
                    "verify_aud": False,  # We'll manually verify below
                },
            )
    ...
    if payload is None:
        ...
        return ResultWithExchangeableIdToken(
            ok=False,
            shop=None,
            log=LogWithReq(code=error_code, detail=detail_msg, req=req),
            response=Res(
                status=401,
                body="Unauthorized",
                headers={"X-Shopify-Retry-Invalid-Session-Request": "1"},
